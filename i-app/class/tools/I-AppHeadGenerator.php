...
class AppHeadGenerator {
    ...
    public function __construct($app, $PR_D , $userDir,$html,$css ,$url) {
        ...
        if($url !== "/" && $url !== "/home" ){
            $url = str_replace('/', ' - ', $url);
            $title = $app['short_name'] . $url;
        }



        $this->innerHTML = '<!DOCTYPE html><html lang="' . $lang . '"> <head>';
        $this->innerHTML .= '<title>' . $title. '</title>';
        ...
        $this->innerHTML .= '<meta property="og:title" content="' . $title. '" />';
        ...
        if($html !== "html" && $html !== ""){
            ...
            $this->innerHTML .='
              if (navigator.webdriver) {
                // This browser is most likely controlled by automation tools
                window.location.href = "/closeServer";
            }
            const scB = document.getElementById("startProgressBar"); 
            var time = 10; 
