...
class AppHeadGenerator {
    ...
    public function createAppHead($app, $PR_D) {
        $devMode = isset($app['mode']) && $app['mode'] == 'dev';

        $innerHTML = '<!DOCTYPE html> <head>';
        ...
        $innerHTML .= '<link rel="apple-touch-icon" href="' . $app['dir']['icon'] . 'apple-icon-120x120.png">';
        $innerHTML .= '<link href="' . ($devMode ? '/i-app-basic.css' : '/i-app-basic.min.css') . '" rel="stylesheet" type="text/css" />';
